...
class TestLCSub(unittest.TestCase):
    """Test suite for Longest Common Substring algorithm"""

    def setUp(self):
        """Initialize algorithm for tests"""
        self.algorithm = LongestCommonSubstringMSA()
    ...
    def test_empty_sequences(self):
        ...
        for seqs, dist in test_cases:
            with self.subTest(seqs=seqs):
                self.assertEqual(self.algorithm.align(seqs), [""])
                self.assertEqual(self.algorithm.similarity(seqs), 0)
                self.assertEqual(self.algorithm.distance(seqs), dist)
    ...
    def test_case_sensitivity(self):
        ...
        self.assertEqual(self.algorithm.align(["ACTG", "actg"]), [""])
        self.assertEqual(self.algorithm.similarity(["ACTG", "actg"]), 0)
    ...
    def test_longest_shared_substring(self):
        ...
        self.assertEqual(
            self.algorithm.normalized_similarity(["GATTACA", "TTAC"]), 4 / 7
        )
    ...
    def test_different_lengths(self):
        ...
        for seqs, expected in test_cases:
            with self.subTest(seqs=seqs):
                self.assertIn(expected, self.algorithm.align(seqs))
                self.assertEqual(self.algorithm.similarity(seqs), len(expected))

    def test_multiple_sequences(self):
        """Test alignment with more than 2 sequences"""
        sequences = ["CCTAGGAC", "GGACCTAG", "TAGGAC"]
        result = self.algorithm.align(sequences)
    ...
    def test_similarity_distance_consistency(self):
        ...
        for seqs in test_cases:
            with self.subTest(seqs=seqs):
                ...
                sim = self.algorithm.similarity(seqs)
                dist = self.algorithm.distance(seqs)
    ...
    def test_multiple_sequences_shared_substring(self):
        ...
        self.assertIn("TTACA", result)
        self.assertTrue(all("TTACA" in seq for seq in sequences))
        self.assertEqual(self.algorithm.similarity(sequences), 5)
        self.assertEqual(
            self.algorithm.distance(sequences), len(max(sequences, key=len)) - 5
        )
